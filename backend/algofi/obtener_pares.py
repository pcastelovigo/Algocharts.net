...
try:
    with open("/scripts/algofi/next-token", "rb") as fichero:
...
def obtenerpool1():
    ...
    obtener_pools = sesion.get('https://algoindexer.algoexplorerapi.io/v2/assets?name=AF-POOL')
...
def obtenerpool2():
    global next_token, hay_next_token
    obtener_pools = sesion.get('https://algoindexer.algoexplorerapi.io/v2/assets?name=AF-POOL&next=' + next_token )
    obtener_pools_p2 = json.loads(obtener_pools.text)
    if 'next-token' in obtener_pools_p2.keys():
        next_token = obtener_pools_p2["next-token"]
        with open("/scripts/algofi/next-token", "wb") as fichero:
...
with open("/scripts/algofi/lista_pares", "wb") as fichero:
