...
try:
    with open("/scripts/next-token", "rb") as fichero:
...
try:
   with open("/scripts/lista_pares", "rb") as fichero:
...
def obtenerpool1():
    ...
    obtener_pools = sesion.get('https://algoindexer.algoexplorerapi.io/v2/assets?name=TinymanPool1.1')
...
def obtenerpool2():
    global next_token, hay_next_token
    obtener_pools = sesion.get('https://algoindexer.algoexplorerapi.io/v2/assets?name=TinymanPool1.1&next=' + next_token )
    obtener_pools_p2 = json.loads(obtener_pools.text)
    if 'next-token' in obtener_pools_p2.keys():
        next_token = obtener_pools_p2["next-token"]
        with open("/scripts/next-token", "wb") as fichero2:
            pickle.dump(next_token, fichero2)
            fichero2.close()
    else:
